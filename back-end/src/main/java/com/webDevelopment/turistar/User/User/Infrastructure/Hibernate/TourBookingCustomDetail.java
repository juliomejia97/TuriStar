...
public class TourBookingCustomDetail implements UserType {
    @Override
    public int[] sqlTypes() {
    ...
    public Object nullSafeGet(ResultSet rs, String[] names, SharedSessionContractImplementor session, Object owner) throws HibernateException, SQLException {
        List<TourBookingDetail> response = null;
        try {
            ...
            if(value.isPresent()) {
                ...
                response = objects.stream().map(tourBooking ->
                        new TourBookingDetail((String) tourBooking.get("id"),
                                (String) tourBooking.get("initDate"),
                                (String) tourBooking.get("endDate"))).collect(Collectors.toList());
            }
        }
        catch (Exception e){
    ...
    public void nullSafeSet(PreparedStatement st, Object value, int index, SharedSessionContractImplementor session) throws HibernateException, SQLException {
        Optional<List<TourBookingDetail>> colors = (value == null) ? Optional.empty() : (Optional<List<TourBookingDetail>>) value;
        try {
            if(colors.isEmpty()) {
                ...
                List<HashMap<String, Object>> objects = colors.get().stream().map(TourBookingDetail::data).collect(Collectors.toList());
